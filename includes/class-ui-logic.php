...
class Fikup_Poly_UI_Logic {
    ...
    public function __construct() {
        ...
        if( is_array($saved_strings) ) {
            foreach($saved_strings as $item) {
                if(!empty($item['org']) && !empty($item['trans'])) {
                    // نرمال سازی: حذف فاصله‌های اضافی و دیکد کردن HTML
                    $key = trim( html_entity_decode( $item['org'], ENT_QUOTES ) );
        ...
        // --- بخش ترجمه (حیاتی) ---
        // اولویت PHP_INT_MAX یعنی "آخرین نفری که نظر می‌دهد"
        // این باعث می‌شود هر چیزی که ووکامرس فارسی تغییر داده را ما دوباره تغییر دهیم
        add_filter( 'gettext', [ $this, 'translate_strings' ], PHP_INT_MAX, 3 );
        add_filter( 'gettext_with_context', [ $this, 'translate_strings_context' ], PHP_INT_MAX, 4 );
        
        // غیرفعال کردن تنظیمات "جایگزینی کلمات" ووکامرس فارسی در حالت انگلیسی
        add_filter( 'option_persian_woocommerce_replacements', [ $this, 'disable_persian_replacements' ] );
        add_filter( 'default_option_persian_woocommerce_replacements', [ $this, 'disable_persian_replacements' ] );
    ...
    /**
     * خنثی کردن تنظیمات جایگزینی کلمات ووکامرس فارسی
     */
    public function disable_persian_replacements( $value ) {
        if ( $this->is_english_context() ) {
            return []; // بازگرداندن آرایه خالی = هیچ کلمه‌ای جایگزین نشود
        }
        return $value;
    }
    ...
    public function translate_strings( $translated, $text, $domain ) {
        ...
        // متن ورودی را تمیز می‌کنیم تا با کلید ذخیره شده مطابقت داشته باشد
        $clean_text = trim( html_entity_decode( $text, ENT_QUOTES ) );
    ...
    public function unload_persian_translations( $mofile, $domain ) {
        ...
        if ( in_array( $domain, $blocked_domains ) ) return ''; 
        return $mofile;
    }
    ...
    public function override_theme_options( $value, $slug ) {
        ...
        $hardcoded_labels = ['mini_cart_view_cart_text', 'mini_cart_checkout_text', 'btn_view_cart_text', 'btn_checkout_text', 'empty_cart_text', 'popup_added_to_cart_message'];
    ...
    public function print_custom_css_and_js() {
        if ( $this->is_english_context() ) {
            ...
            echo '<style>';
            echo '
                body.fikup-en-mode, body.fikup-en-mode h1, body.fikup-en-mode h2, body.fikup-en-mode h3, 
                ...
                body.fikup-en-mode span, body.fikup-en-mode div, body.fikup-en-mode strong, body.fikup-en-mode b, 
                body.fikup-en-mode i, body.fikup-en-mode em, body.fikup-en-mode button, body.fikup-en-mode input, 
                body.fikup-en-mode textarea, body.fikup-en-mode select, body.fikup-en-mode label,
                ...
                body.fikup-en-mode [class*="wd-icon-"], body.fikup-en-mode .woodmart-font,
                body.fikup-en-mode .wd-tools-icon, body.fikup-en-mode .wd-cross-icon,
                body.fikup-en-mode .wd-arrow-inner, body.fikup-en-mode .social-icon,
                body.fikup-en-mode .star-rating, body.fikup-en-mode .star-rating span:before {
